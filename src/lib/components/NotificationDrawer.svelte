<script lang="ts">
  ...
  import { deleteNotification, notification_open, notifications } from "$lib/stores/Dialogs";
  ...
  async function handleNotificationClick(n: Notification, action: string) {
        ...
        {#each $notifications as n}
          <Card>
            ...
            <Actions>
              ...
              {/each}
              <ActionIcons>
                <IconButton class="material-icons" on:click={() => deleteNotification(n.tag)}>
                  close
                </IconButton>
              </ActionIcons>
            </Actions>
